...
const ViewOfficerAdmin = () => {
  ...
  useEffect(() => {
   ...
   const filteredData = complaintData.filter((complaint) =>{ 
        const isMatch = Object.values(complaint).some((field) =>
        ...
        <div className="section-heading d-flex align-items-center justify-content-between">
          ...
          <div className="row g-3" >
            ...
            <div className="row" style={{marginTop:10}}>
              ...
              <div key={officer._id} className="col-12 col-md-6">                                        
        
              <div className="card product-card" style={{marginBottom:10}}>
                <div className="card-body">
                      <a className="product-title d-block"  >Email:  <b> {officer.email} </b></a>
                      <a className="product-title d-block"  >Name :  <b>  {officer.name} </b></a>
                      <a className="product-title d-block"  >Contact no:{officer.mobile}  </a>
                      <a className="product-title d-block"  >Location:{officer.location}  </a>
                      {/* <a className="product-title d-block"  >Department:{officer.department}  </a> */}
                      <a className="product-title d-block">Date:{new Date(officer.dateCreated).toLocaleDateString('en-GB',timeOptions)}  </a>

                    </div>
                  </div>   
                       
                  <a className="btn btn-danger" onClick={() => { LoadEdit(officer.id) }}>Update</a>
                  <a className="btn btn-danger" onClick={() => { Removefunction(officer.id) }}>Delete</a>
